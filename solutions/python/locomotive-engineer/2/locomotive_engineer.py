...
def fix_list_of_wagons(each_wagons_id, missing_wagons):
    ...
    """
    wagon1, wagon2, wagon3, *balance_ids = each_wagons_id
    res = []
    res.append(wagon3)
    return res + missing_wagons + balance_ids + list((wagon1, wagon2))
...
def add_missing_stops(route, **kwargs):
    ...
    """
    stop_list = [value for key, value in kwargs.items()]
    route.update({"stops": stop_list})
    return route
...
def fix_wagon_depot(wagons_rows):
    ...
    print(result)
    return [list(column) for column in result]
